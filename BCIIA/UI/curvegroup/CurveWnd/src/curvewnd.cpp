...
void CurveWnd::update()
{
    ...
    maxwidget->setPoints(x,y,seriesdata.at(max_id)->max(),seriesdata.at(max_id)->min());
...
void CurveWnd::appendMark(QString event)
{
    mark.append(event,current_pos);
}

void CurveWnd::append(QList<double> data)
{
    if(data.size()!=seriesdata.size())
    {
        return;
    }
    mark.updateMarkData();
